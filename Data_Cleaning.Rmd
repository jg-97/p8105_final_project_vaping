...
df = df %>% 
    ...
    rename(biological_sex = sexid) %>%
  ### Changed so that the distinction btw 2 sid is more cleared_HS
    # Sexual Contact
    mutate(
      q66 = recode(q66,
                   "1" = "no sexual contact", 
                   ...
                   "4" = "females and males"),
      q66 = as.factor(q66)) %>% 
    rename(sexual_contact_target = q66) %>% 
  ###Changed so that the distinction btw 2 sct is more cleared_HS
    # Sexual Contact 2
    mutate(
    ...
    rename(sexual_contact_scope = sexpart) %>% 
#####################################################################################
...
The following secctions get the percentage of NA and the distribution of all response among all years and all question into the data cleaning rmd, after the undesired column is removed, can get new result by simply rerun the following codes.
...
##This table shown the amount of different response per column per year
distribution 

map_df(df_total,fill_NA)%>%mutate(id = as.numeric(as.character(id)),year = as.numeric(as.character(year)))%>%
  select(-"id")%>%
  pivot_longer(names_to = "question",values_to = "response",cols = 3:ncol(.))%>%
  count(borough,year,question,response)%>%group_by(borough,year,question,response)%>%
  summarise(sum(n))%>%left_join(x = . , y = df_total%>%group_by(year)%>%summarise(n()))%>%
