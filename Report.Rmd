...
* 81% of Americans report teenagers who would not smoke cigarettes are using flavored e-cigarettes.[[2](https://www.kff.org/other/issue-brief/data-note-vaping-and-e-cigarettes/)]

* One pod of the most popular e-cigarette product -JUUL- contains 20 cigarettes worth of nicotine.[[3](https://truthinitiative.org/research-resources/emerging-tobacco-products/how-much-nicotine-juul)]



According to reports in recent years, an underlying trend of increased vaping, especially amoung teenagers, can be observed.
...
3. How do drug use and mental health status differ by demographics in NYC's youth?
...
* Staten Island had the highest Marijuana use over the years among all five NYC boroughs. However, in year 2017, it dropped dramatically and had the lowest proportion among all boroughs. 
...
## Exploring violent and risky behaviors

We were interested in whether violent and risky behaviors differ among teenagers using a particular drug, compared to teenagers who do not use a certain drug. To that end, we created the following animated plot:
...
Studying this plot, we were able to derive the following results:

* The left plot has twice the value than the right plot when the bar is in the same height. Therefore, the proportion of having violence nad risky behaviors in the teenagers who use drugs is nearly twice than the that in the teenagers who are non-drug users. Moreover, non-drug users have lower proportion in carrying weapons from 2003 to 2017. 
...
# Discussion
<br>
Vaping, no doubt is getting not just press attention, but is a public health crisis in youth. How to use current available data to prevent future vaping use in youth is an urgent task for public health researchers. After visualizing the YRBSS data, we proposed three different models using data in 2015 to predict youth vaping use in 2017. We used prediction accuracy in cross-validation (CV) as the criteria to choose the best predicting model. By conducting a 5 folds 10 times CV, the model that generated by StepAIC outstood the manually and LASSO selected models. The StepAIC model picked 10 predictors (carring_weapon, sad_hopeless, attempted_suicide, safety_concerns_at_school, physical_fighting, bullying_electronically, age, ace7, illegal_injected_drug_use and sexual_contact_2) and gives an overall of 87% accuracy rates. Teenagers with depressive mood, suffering depression, have problems at school and involve with risky behaviors are more like to start vaping at a young age. 

<br> <br>

The limitation of this dataset is that we only have vaping data for two years. In addition to that, we have to drop some potential predictors due to high level of missingness.  Despite these, our findings are important. We wish to apply our methods to data from 2018 and 2019 when they become publicly available. Should the prediction accuracy rate stay high, it will help policy makers, schoolteachers, parents to work together to focus on targeted population to improve efficiency in reducing vaping rate in youth. 

<br> <br>

We thank Dr. Jeff Goldsmith and his TA team for helping us and gave us this opportunity to work together to actually trying to solve some real public health issues. We hope that you enjoy watch our screencast and the work that we presented. 
<br>
