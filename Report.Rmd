...
First, we attempted to find a predictive model by using a variation on **stepwise/automatic procedures** (by hand). We utilized p-values and prediction accuracy as our guidance for which predictors to choose, starting with the full model. In the end, this process came up with the following possible model:
...
With the three models generating methods at hand, we decide to use prediction accuracy in cross-validation as the criteria to choose which of them can predict the vaping status of New York teenagers based on self-reported information that the majority of them willing to share. The prediction accuracy is calculated as the proportion of correct prediction made by the model.
...
By conducting a 5 fold 10 times CV on the three models, according to the violin plot showing the distribution of prediction accuracy, The models that generated by stepAIC(StepAIC model) have overall approximately 1% and 2% better accuracy compare to the models that generated by lasso (Lasso model) and the manual selections (Manual model) methods,respectfully. And therefore, the stepAIC model is of our choosing.




Consequentially, when applying the three models to “forecast” the vaping status of teenagers in 2017, the StepAIC model also performed the best out of the three, gives a more than 87% accuracy rates. Following are the formula that determined by the StepAIC model:
