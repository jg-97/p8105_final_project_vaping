...
nyc_drug <- df_total
####A bette way to do it by avoiding uneeded format transformation_byHS
```
...
drug_p <- marijuana_full %>%
  filter(!(is.na(frequency)))%>%ggplot(aes(x = year, y = frequency, color = borough, shape = drug_type, 
